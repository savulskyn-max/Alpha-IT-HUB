...
export default function GastosAnalyticsPage() {
  ...
  const pieData = data?.por_tipo && data.por_tipo.length > 0
    ? data.por_tipo.map((x) => ({ nombre: x.tipo, total: x.total, pct: x.pct }))
    : [];
  ...
  return (
    <div className="flex flex-col flex-1">
      <div className="bg-[#1E3340] border-b border-[#32576F] px-6 py-4 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-lg font-semibold text-white">Analitica - Gastos</h1>
          <p className="text-[#7A9BAD] text-sm">Clasificacion por tipo y detalle ordenado por fecha</p>
        </div>
      </div>
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              ...
              <KpiCard
                ...
                sub={data.ratio_ventas != null ? 'del revenue del periodo' : undefined}
              />
              <KpiCard
                label="Promedio diario"
                value={fmt(data.serie_temporal.length > 0 ? data.total_periodo / data.serie_temporal.length : 0)}
                sub="por dia con gastos"
              />
            </div>

            <ChartContainer title="Gastos por dia" exportFileName={`gastos_serie_${tenantId}`}>
              <ResponsiveContainer width="100%" height={240}>
                <LineChart data={data.serie_temporal} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                  ...
                  <Tooltip contentStyle={{ background: '#132229', border: '1px solid #32576F', borderRadius: 8 }} formatter={(v: number | undefined) => [fmt(v ?? 0), 'Gastos']} />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <ChartContainer title="Gastos por tipo" exportFileName={`gastos_tipo_${tenantId}`}>
                {pieData.length > 0 ? (
                  <ResponsiveContainer width="100%" height={260}>
                    <PieChart>
                      <Pie data={pieData} dataKey="total" nameKey="nombre" cx="50%" cy="50%" outerRadius={95} innerRadius={50} paddingAngle={2}>
                        {pieData.map((_, idx) => <Cell key={idx} fill={COLORS[idx % COLORS.length]} />)}
                      </Pie>
                      <Tooltip contentStyle={{ background: '#132229', border: '1px solid #32576F', borderRadius: 8 }} formatter={(v: number | undefined, _n, p) => [fmt(v ?? 0), `${p.payload.nombre} (${p.payload.pct}%)`]} />
              ...
              <ChartContainer title="Gastos por metodo de pago" exportFileName={`gastos_metodo_${tenantId}`}>
            ...
            <ChartContainer title="Detalle de gastos" subtitle="Incluye fecha, tipo y categoria" exportFileName={`gastos_detalle_${tenantId}`}>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-[#32576F]">
                      {['Fecha', 'Categoria', 'Tipo', 'Metodo', 'Monto'].map((h) => (
                  ...
                  <tbody>
                    {(data.detalle ?? []).map((row, i) => (
                      <tr key={i} className="border-b border-[#32576F]/40 hover:bg-[#132229] transition-colors">
                        <td className="py-2 px-3 text-white">{row.fecha}</td>
